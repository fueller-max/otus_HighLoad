resource "proxmox_vm_qemu" "lab04-backend3" {
    name = "lab04-backend3"
    description = "lab04_backend3"
    ...
    network {
     ...
     # Lan4: internal network (backend <-> iSCSI)
    network {
        id = 3
        model = "virtio"
        bridge = "vmbr4"
    ...
    ipconfig0 = "ip=192.168.70.45/24,gw=192.168.70.1"
    nameserver = "8.8.8.8"
    ciuser = "deploy"
    sshkeys = <<EOF
       ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEwOGqPyDBruydkg1DPItwaBBwo5/5gAaBYeshgNFmlS maksim@maksim-asus-tuf
     EOF
    ipconfig1 = "ip=10.10.20.45/24"
    ipconfig2 = "ip=10.10.30.45/24"
    ipconfig3 = "ip=10.10.40.45/24"
